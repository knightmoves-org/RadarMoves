...
public class RadarDataController(IRadarDataProvider dataProvider, ILogger<RadarDataController> logger) : ControllerBase {
    ...
    public async Task<IActionResult> GetMetadata() {
        try {
            var timeRange = await _dataProvider.GetTimeRangeAsync();
            var timestamps = await _dataProvider.GetTimestampsAsync();
            var timestampList = timestamps.ToList();

            return Ok(new {
                timeRange = timeRange.HasValue ? new {
                    start = timeRange.Value.Start,
                    end = timeRange.Value.End
                } : null,
                count = timestampList.Count,
                channels = _dataProvider.GetAvailableChannels().Select(c => c.ToString()).ToArray()
            });
        } catch (Exception e) {
    ...
    public async Task<IActionResult> GetElevations(string timestamp) {
        try {
            ...
            var elevations = await _dataProvider.GetElevationAnglesAsync(dt);
            if (elevations == null) {
    ...
    public async Task<IActionResult> GetScanMetadata(string timestamp, float elevation) {
        try {
            ...
            var metadata = await _dataProvider.GetScanMetadataAsync(dt, elevation);
            if (metadata == null) {
    ...
    public async Task<IActionResult> GetValue(string channel, string timestamp, float elevation, int ray, int bin) {
        try {
            ...
            var value = await _dataProvider.GetValueAsync(ch, dt, elevation, ray, bin);
            if (!value.HasValue) {
    ...
    public async Task<IActionResult> GetValueByGeo(string channel, string timestamp, float elevation, float latitude, float longitude) {
        try {
            ...
            var value = await _dataProvider.GetValueByGeoAsync(ch, dt, elevation, latitude, longitude);
            if (!value.HasValue) {
                return NotFound(new { error = "Value not found" });
            }
    ...
    public async Task<IActionResult> GetImage(string channel, string timestamp, float elevation, int width = 1500, int height = 1500) {
        try {
            if (!Enum.TryParse<Channel>(channel, true, out var ch)) {
                return BadRequest(new { error = "Invalid channel" });
            }
            ...
            var imageBytes = await _dataProvider.GetImageAsync(ch, dt, elevation, width, height);
